...
Ви можете імпортувати готовий шаблон і створити із нього автоматизацію, яка примусово оновлюватиме дані частіше. Виберіть створені Scrape-сенсори та інтервал оновлення.
...
Ось пояснення як ці сутності працюють:
...
Імпортуйте готовий шаблон для критичних сповіщень і створіть із нього автоматизацію:

[![Імпортувати шаблон для критичних сповіщень][blueprint-install-image]][critical-notification-blueprint-install-url]

Виберіть пристрій Aerial Danger (Повітряна загроза), пристрій із застосунком Home Assistant і затримку між повторними сповіщеннями.
...
- **Чому інтеграція не читає дані самостійно?** — Джерела даних для різних регіонів та місцевостей можуть відрізнятись. Користувач самостійно обирає найбільш релевантні джерела даних для свого регіону. Завдання інтеграції — лише аналізувати текст на наявність загроз.
- **Чому не аналізувати повідомлення з допомогою штучного інтелекту?** — ШІ-моделі недетерміністичні. Вони можуть толерувати небезпеки, або навпаки — частіше реагувати на безпечні повідомлення. Окрім того, запит, обробка та відповідь ШІ-моделі потребують додаткових запитів та часу, що може призвести до неприпустимих затримок. Регулярні вирази та пошук ключових слів хоч і примітивніші, але передбачувані та дозволяють майже миттєву обробку повідомлень.
- **Мого регіону або місцевості немає у списку готових. Як їх додати?** — Для початку, підберіть та додайте власні [регулярні вирази][python-regex-url] у налаштуваннях інтеграції. Намагайтесь підібрати такий вираз, який буде знаходити різні варіації написання вашої місцевсті (наприклад `київ`, `києва`, `києві`, `києвом`). Підібрати і тестувати власний вираз можна з допомогою [пісочниць для регулярних виразів](https://regexr.com/). Якщо у вас вийде підібрати хороший вираз, додайте його до [переліку місцевостей][presets] та надішліть його у вигляді [пул-реквесту до репозиторію інтеграції][contributing], щоб інші користувачі могли ним скористатись.
- **Джерело порожнє або `unavailable`. Що робити?** — Перевірте URL, CSS-селектор і доступність публічного каналу без авторизації.
- **Загрози не виявляється.** — Перевірте, чи стан джерела містить повний текст, а повідомлення згадує вибраний регіон або місцевість.
- **Оновлення каналів ствоорює надто багато запитів.** — Збільште інтервал оновлення Scrape до 10 секунд і залиште лише потрібні канали.
- **Як змінити налаштування інтеграції?** — Відкрийте **Налаштування** запису інтеграції та оновіть пресети або власні регулярні вирази.
